...
class MythicRPC(MythicBaseRPC):
    async def get_functions(self) -> RPCResponse:
        ...
        try:
            ...
            return RPCResponse(
                {
                    "status": "error",
                    "error": "Failed to find function and call it in RPC, does it exist?\n"
                    + str(e),
                }
            )
    ...
    async def execute(self, function_name: str, **func_kwargs) -> RPCResponse:
        ...
        try:
            ...
            sys.stdout.flush()
            return RPCResponse(
                {"status": "error", "error": "Failed to call function:\n" + str(e)}
            )
    ...
    async def execute_c2rpc(
        self, c2_profile: str, function_name: str, message: str, task_id: int
    ) -> RPCResponse:
        try:
            resp = await self.call_c2rpc(
                {"action": function_name, "message": message, "task_id": task_id},
                c2_profile,
            )
            return resp
